...
try:
    ...
    st.sidebar.warning("Network intrusion model not found. Run src/train_model.py first.")
...
# ----------------------------- SAVE LOG ------------------------------------
def save_transaction(user_name, password_ok, answer_ok, amount, amount_ok, score, decision, message):
    log_entry = {
...
with left_col:
    ...
    if st.session_state.blocked_until and datetime.now() < st.session_state.blocked_until:
        st.error(f"❌ Too many failed attempts. Locked until {st.session_state.blocked_until.strftime('%H:%M:%S')}")
    ...
    with st.form("transaction_form"):
        ...
        submit = st.form_submit_button("Submit Transaction", disabled=transaction_disabled)
    ...
    if submit and not transaction_disabled:
        ...
        save_transaction(user_name, password_ok, answer_ok, amount, amount_ok, score, decision, message)
        ...
        if decision == "ALLOWED":
            ...
            if st.session_state.attempts >= 2:
                st.session_state.blocked_until = datetime.now().replace(second=0, microsecond=0) + pd.Timedelta(minutes=5)
...
if not df.empty:
    ...
    if len(df) > 1:
        ...
        fig_line = px.line(df_sorted, x="timestamp", y="threat_score",
                           color="decision", markers=True,
                           title="Threat Score Over Time",
                           labels={"threat_score": "Threat Score (0-1)", "timestamp": "Time"})
        fig_line.add_hline(y=0.8, line_dash="dash", line_color="red", annotation_text="Block (0.8)")
        fig_line.add_hline(y=0.5, line_dash="dash", line_color="orange", annotation_text="Flag (0.5)")
    ...
    if not high_risk.empty:
        st.dataframe(high_risk[["timestamp", "user", "amount_requested", "threat_score", "decision", "alert_message"]],
...
if not df.empty:
    ...
    st.download_button("📥 Download Full Logs (CSV)", data=csv, file_name="bank_logs.csv", mime="text/csv")
